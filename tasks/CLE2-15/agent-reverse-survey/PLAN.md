...
- 실행 과제, 증거, 검증 여부
- 주간 점검 기록
- 삭제된 설문을 참조하는 고아 회차 감지

#### 6-2. 보안 기준

- 모든 통합 조회 요청에 `X-API-Key` 필요
- API 키를 HTML·JavaScript에 내장하지 않음
- `localStorage`·`sessionStorage`에 API 키를 저장하지 않음
- 조회 요청 완료 후 입력 필드에서 API 키 제거
- 비밀번호 해시는 관리자 응답에서도 제외
